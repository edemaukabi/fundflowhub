...
import { Eye, EyeOff, Loader2, Moon, Sun, ShieldCheck } from "lucide-react";
...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    setServerError(null);
    try {
      await authApi.resetPassword({ uid, token, new_password: values.new_password });
      setDone(true);
          ...
          <div className="w-full max-w-sm">
            <div className="mb-8 flex flex-col items-center text-center">
              <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-ffh-teal/10">
                <ShieldCheck size={26} className="text-ffh-teal" />
              </div>
            ...
            {!done && (
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  {errors.new_password && (
                    <p className="text-xs text-ffh-danger">{errors.new_password.message}</p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.confirm_password && (
                    <p className="text-xs text-ffh-danger">{errors.confirm_password.message}</p>
                  )}
                </div>
